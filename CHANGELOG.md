...
- **`device_info` now supplies `name`, `manufacturer`, and `model`.** Both `RingExtendedSensor.device_info` and `RingDeviceFirmwareHistorySensor.device_info` previously returned `identifiers` only, on the assumption (stated in a code comment since the first release) that this merely *links* to the device row created by the base `ring` integration. Current HA no longer allows two config entries to share a device row: it migrated each shared "composite" device into **one row per config entry**, copying name/model/area/`created_at` to both halves. So `ring_extended` has its own row per camera, alongside base `ring`'s — confirmed on a live instance via `config/device_registry/list_composite_splits`, which reports 21 split composites, each yielding a `ring` row (7-10 entities) and a `ring_extended` row (300+ entities). The grandfathered rows inherited a name from the composite, which masked the defect; any device added to the Ring account *after* the migration has no composite to inherit from, so it got a `ring_extended` row with `name: null` — rendered as "Unnamed device" with several hundred sensors under it.
